...
def validate_skill(skill_path, platform=PLATFORM_AUTO, strict_openai_yaml=False):
    ...
    name = name.strip()
    if name:
        # Codex bundled skills include uppercase display-like names. A single colon
        # separates an optional author namespace from the skill's local name.
        if not re.match(r'^[A-Za-z0-9-]+(?::[A-Za-z0-9-]+)?$', name):
            return False, (
                f"Name '{name}' should use letters, digits, and hyphens with at most "
                "one namespace colon"
            )
        if any(part.startswith('-') or part.endswith('-') or '--' in part for part in name.split(':')):
            return False, (
                f"Name '{name}' segments cannot start/end with hyphen or contain consecutive hyphens"
            )
