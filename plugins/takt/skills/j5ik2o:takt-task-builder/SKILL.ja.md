...
このタスクで `.takt/workflows/*.yaml` を編集した場合は、完了判定前に `j5ik2o:takt-workflow-builder` スキルのバリデーションスクリプトを実行する。
スキルの配置先に応じたパスで実行する:

```bash
bash <j5ik2o:takt-workflow-builderスキルのディレクトリ>/scripts/validate-takt-files.sh --workflows
```
