...
If this task edits `.takt/workflows/*.yaml`, run the `j5ik2o:takt-workflow-builder` skill's validation script before completion judgment.
Execute using the path appropriate for the skill's deployment location:

```bash
bash <j5ik2o:takt-workflow-builder skill directory>/scripts/validate-takt-files.sh --workflows
```
