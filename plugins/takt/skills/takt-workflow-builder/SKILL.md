...
**`provider_options.$ref` による共通化（v0.46.0〜）**: `provider_options` はワークフローファイルからの相対パスで共通 YAML を参照できる。inline 値が同じ leaf を上書きする。ビルトインプリセット（`provider-options/edit.yaml`, `review-readonly.yaml`, `review-files.yaml`, `review-web.yaml`）も利用可能。
...
| `provider_options.$ref` | ワークフローファイルからの相対パスで共通 YAML を参照（inline 値が leaf を上書き） |
