...
| OTLP span / metric（v0.46.0〜） | span / metric 単位 | `OTEL_EXPORTER_OTLP_ENDPOINT` 設定時に Grafana Tempo 等へ送信。provider span が workflow trace にネストされるため、step ↔ provider 呼び出しの対応をトレースビューで確認可能 |
