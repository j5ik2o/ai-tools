...
| スキル | 説明 |
|--------|------|
| `j5ik2o-takt-task-builder` | TAKT の `tasks.yaml` エントリと `.takt/tasks/{slug}/order.md` タスクディレクトリを作成・編集 |
| `j5ik2o-takt-workflow-builder` | TAKT のワークフロー YAML と関連ファセットを作成・カスタマイズ |
| `j5ik2o-takt-facet-builder` | Persona / Policy / Instruction / Knowledge / Output Contract などの TAKT ファセットを個別に作成・編集 |
| `j5ik2o-takt-analyzer` | 既存の TAKT ワークフロー、facet、実行ログを分析し、問題点と改善案を提示 |
| `j5ik2o-takt-optimizer` | 既存の TAKT ワークフローをトークン消費、ルール構成、実行フローの観点で最適化 |
| `j5ik2o-takt-skill-updater` | `references/takt` サブモジュール更新後に `takt-*` スキル群を追従更新 |
