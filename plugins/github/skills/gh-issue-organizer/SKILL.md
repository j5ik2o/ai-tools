...
> ここでは「自動生成 / 手動報告」の2区分のみで概況を把握する（合計 ＝ CodeRabbit 自動生成 ＋ 手動報告）。バグ・機能要求・Epic / リファクタリング・その他の内訳はステップ2で分類し、ステップ5で集計する。
...
| **その他** | 上記に該当しない手動報告（人間が作成し、バグ・機能要求・Epic のいずれにも該当しないもの） |

> 分類は上から順に判定し、最初に一致したカテゴリを採用する（先勝ち）。**CodeRabbit 自動生成** 以外（＝人間が作成した手動報告）を、バグ → 機能要求 → Epic / リファクタリング の順で判定し、いずれにも該当しないものを **その他** とする。「人間が作成」は手動報告かどうかの判定にのみ用い、バグ判定には用いない（バグ判定はラベル `bug` とタイトルで行う）。
...
2. **対象コードの存在確認**: ファイル・シンボルが現在のコードベースに存在するか確認する
   ...
   ```
   - ファイルが存在しない → **対象消失**（理由: 対象コード削除済み）
   - シンボルが存在しない → **対象消失**（理由: 対象シンボル削除済み）
